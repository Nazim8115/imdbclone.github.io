...
(function () {
  ...
  const runtime = document.getElementById("duration");
  ...
  async function fetchMovies(search) {
    const url = `https://www.omdbapi.com/?t=${search}&type=movie&apikey=e34ccfe1`;
    try {
      const response = await fetch(url);
      const data = await response.json();
      console.log(data);
      title.innerHTML = data.Title; //change this
